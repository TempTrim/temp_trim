...
def tram_inputs(i):
    ...
    with tabs[i]:
        ...
        with st.container():
            ...
            width = st.number_input("Width [m]", key=f"width_{i}", help = "Width of tram (external)")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "width"], key=f"default_width_{i}")
            height = st.number_input("Height [m]", key=f"height_{i}", help = "Height of tram (external)")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "height"], key=f"default_height_{i}")
            passenger_count = st.number_input("Average passenger count", key=f"passenger_count_{i}", help = "Average tram occupancy during an operating day")
