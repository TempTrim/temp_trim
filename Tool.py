...
def tram_inputs(i):
    ...
    with tabs[i]:
        ...
        with st.container():
            ...
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "name"], key=f"default_name_{i}", help = "Unique Name for the Tram model")
            tram_count = st.number_input("Tram count", min_value = 1, value = 1, step = 1, key=f"tram_count_{i}")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "tram_count"], key=f"default_tram_count_{i}")
            tram_frac = st.number_input("Fraction of trams in operation", key=f"tram_frac_{i}", help = "Fraction of trams in operation (0-1)")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "tram_frac"], key=f"default_tram_frac_{i}")
            length = st.number_input("Length [m]", key=f"length_{i}", help = "Total length of tram")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "length"], key=f"default_length_{i}")
            width = st.number_input("Width [m]", key=f"width_{i}", help = "Width of tram internal space")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "width"], key=f"default_width_{i}")
            height = st.number_input("Height [m]", key=f"height_{i}", help = "Height of tram internal space")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "height"], key=f"default_height_{i}")
            passenger_count = st.number_input("Average passenger count", key=f"passenger_count_{i}", help = "Average tram occupancy during an operation day")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "passenger_count"], key=f"default_passenger_count_{i}")
            ventilation = st.number_input("Average fresh air supply via ventilation [m³/h]", key=f"ventilation_{i}", help = "Average air flow = 0.5 * time_fraction_doors_open * average_area_open_doors * air_speed_through_door. Where: time_fraction_doors_open: fraction of time during which doors are usually open (0-1), average_area_open_doors: the average area of doors that are opened [m²], air_speed_through_door: measured air velocity through an open door [m/s]]")
            ...
            convection = st.number_input("Average heat transfer coefficient at chassis [W/(m²K)]", key=f"convection_{i}", help = "Combined heat transfer coefficient for convection at and conduction through the chassis, where coefficient_combined = coefficient_convection*coefficient_conduction/(coefficient_convection + coefficient_conduction)")
            st.button("Load Cobra VBZ value", on_click=defaults, args = [i, "convection"], key=f"default_convection_{i}")
            auxillary_heat = st.number_input("Auxillary heat generated in tram [W]", key=f"auxillary_heat_{i}", help = "Heat generated by other devices in tram interior except for heaters and traction system (e.g. lighting, displays, etc.)")
...
if calc_button:
    ...
    if len(heater_inputs) < tram_count or T_tram == []:
        ...
        try:
            ...
            elec_inst = consumption.electricity_instantaneous
            index_tuples = []
            values = []

            def get_output_csv(inst, type):
                ...
                if type == "heat":
                    ...
                    heat_type_mapper = {0: 'Heating Power', 1: 'Solar Heating', 2: 'Passenger Heat', 3: 'Auxillary Heating', 4: 'Convective Losses', 5: 'Ventilation Losses', 6: 'Open Door Losses'} 
                    ...
                    df_elec = df_elec.replace({"Temperature": temp_mapper})
                    df_elec = df_elec.replace({"Electricity_Type": elec_type_mapper})
                    df_elec = df_elec.replace({"Tram": tram_mapper})

                    return df_elec
                
            df_heat = get_output_csv(heat_inst, "heat")
